...
def test_proper_complex_start():
    ...
    order = {}
    Ordering().calculateDependenciesStart(data, order)
    assert order == {'dashboard': 4, 'elasticsearch': 0, 'fullhouse': 3, 'hadoop-namenode': 1, 'hbase-master': 2, 'kibana': 1, 'mysql': 0, 'zookeeper': 0}

def test_proper_complex_stop():
    data = DataLoader().load('fixtures/bh-example.yml')
    order = {}
    Ordering().calculateDependenciesStop(data, order)
    assert order == {'dashboard': 0, 'elasticsearch': 2, 'fullhouse': 1, 'hadoop-namenode': 3, 'hbase-master': 2, 'kibana': 0, 'mysql': 2, 'zookeeper': 4}
